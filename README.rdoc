...
*NOTE* if you have a previous version of ShinyCocos and have a "ruby"
directory, remove that directory and pull the latest version from the
repository (or completly remove ShinyCocos and pull the latest version).

First, you need to get the latest release of cocos2d (currently, working with v0.8 from trunk)
You can either fetch it from the project page or using subversion:

  svn co http://cocos2d-iphone.googlecode.com/svn/trunk cocos2d-iphone

If you already have a copy of Cocos2D-iphone lying around, you can
safely create a symbolic link with the name of "cocos2d-iphone":

  cd shinycocos
  ln -s /path/to/cocos2d-iphone cocos2d-iphone
